...
CREATE OR REPLACE PROCEDURE  ADD_COMMENT(C_ID IN NUMBER,S_ID IN NUMBER,STARS IN NUMBER,COM IN VARCHAR2)
IS
C_COUNT NUMBER;

BEGIN

SELECT COUNT(*) INTO C_COUNT FROM FEEDBACK WHERE COURSE_ID=C_ID AND STUDENT_ID=S_ID;
IF C_COUNT >0 THEN 
UPDATE FEEDBACK SET RATING =STARS, FEEDBACK_COMMENT=COM ,FEEDBACK_DATE=SYSDATE WHERE STUDENT_ID =S_ID AND COURSE_ID=C_ID;
ELSE 
INSERT INTO FEEDBACK(FEEDBACK_ID,COURSE_ID,STUDENT_ID,FEEDBACK_COMMENT,RATING,FEEDBACK_DATE) VALUES(FEEDBACK_SEQ.NEXTVAL,C_ID,S_ID,COM,STARS,SYSDATE);
END IF;
